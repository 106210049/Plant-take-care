...
RTC_DATA_ATTR int state = 1;                      // Biến state sẽ xoay vòng giữa 1, 2, 3
...
RTC_DATA_ATTR bool mode_tmp=false;
unsigned long lastBlinkTime;
volatile long lcdBlinking=false;
//volatile long buzzerStartTime;
RTC_DATA_ATTR bool emergencyMode = false;         // Emergency mode status
...
const int sleepDuration = 10000000; // Set sleep duration in microseconds (e.g., 5 minutes)
RTC_DATA_ATTR bool motorState=false;
...
void Load::Control_Motor() {
  if(sensor.target_value-sensor.moisturePercent<=5){
    Stop_Motor();
  }
  else{
    motorState=true;
